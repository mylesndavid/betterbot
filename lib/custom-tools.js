  ...
  for (const file of jsFiles) {
    try {
      ...
      if (!tool?.name || !tool?.execute) {
        await quarantine(file, 'Missing name or execute function');
        continue;
      }

      // Check for collision with built-in tools
      if (builtinNames.has(tool.name)) {
        await quarantine(file, `Name "${tool.name}" conflicts with a built-in tool`);
        continue;
      }

      // Check for duplicate custom tool names
      if (seenNames.has(tool.name)) {
        await quarantine(file, `Duplicate tool name "${tool.name}" (already loaded from another file)`);
        continue;
      }
      ...
      if (schemaErrors.length > 0) {
        await quarantine(file, `Invalid schema:\n${schemaErrors.join('\n')}`);
        continue;
      }
      ...
      await quarantine(file, `Load error: ${err.message}`);
    }
  }
