...
import { getUpcoming, createEvent, quickAdd, updateEvent, deleteEvent, formatEvents } from './google-calendar.js';
...
import { notifyUser, notifyOwnerFromBf } from './notify.js';
import { sendToFriend, listFriends as listBetterFriends } from './channels/betterfriends.js';
...
import { listRules, addRule, removeRule } from './rules.js';
...
// --- Git repo locator (self-contained, same pattern as gateway.js) ---
const __tools_dirname = dirname(fileURLToPath(import.meta.url));
const TOOLS_APP_ROOT = join(__tools_dirname, '..');

let _cachedRepoDir = null;
function findRepoDir() {
  if (_cachedRepoDir !== null) return _cachedRepoDir;
  const candidates = [
    TOOLS_APP_ROOT,
    join(homedir(), 'Development/Repos/betterclaw'),
    join(homedir(), 'Development/Repos/betterbot'),
    join(homedir(), 'betterclaw'),
    join(homedir(), 'betterbot'),
  ];
...
const tools = [
  ...
  {
    name: 'send_imessage',
    description: 'Send an iMessage to a phone number, email, or contact name via the macOS Messages app. Requires Automation permission for Messages.',
    parameters: {
      type: 'object',
      properties: {
        recipient: { type: 'string', description: 'Phone number (e.g. +14155551234), email address, or contact name' },
    ...
    async execute(args) {
      ...
      const recipient = args.recipient.replace(/\\/g, '\\\\').replace(/"/g, '\\"');
      const message = args.message.replace(/\\/g, '\\\\').replace(/"/g, '\\"');
      try {
        execSync(`osascript -e 'tell application "Messages" to send "${message}" to buddy "${recipient}" of (1st account whose service type = iMessage)'`, {
          encoding: 'utf-8',
          timeout: 15000,
        });
        return `iMessage sent to ${args.recipient}.`;
      } catch (err) {
        // Common fallback: try without specifying account
        try {
          execSync(`osascript -e 'tell application "Messages" to send "${message}" to buddy "${recipient}"'`, {
            encoding: 'utf-8',
            timeout: 15000,
          });
          return `iMessage sent to ${args.recipient}.`;
        } catch (err2) {
          return `Failed to send iMessage: ${err2.message}`;
        }
      }
    },
  },
  ...
  {
    name: 'read_imessages',
    description: 'Read recent iMessages from the local Messages database (chat.db). Can filter by contact. Requires Full Disk Access permission.',
    parameters: {
      type: 'object',
      properties: {
        contact: { type: 'string', description: 'Phone number or email to filter by (optional — omit for all recent messages)' },
    ...
    async execute(args) {
      ...
      let query;
      if (args.contact) {
        const contact = args.contact.replace(/'/g, "''");
        query = `SELECT m.rowid, datetime(m.date/1000000000 + 978307200, 'unixepoch', 'localtime') as timestamp, m.is_from_me, h.id as contact, m.text FROM message m LEFT JOIN handle h ON m.handle_id = h.rowid WHERE h.id LIKE '%${contact}%' AND m.text IS NOT NULL ORDER BY m.date DESC LIMIT ${limit}`;
      } else {
        query = `SELECT m.rowid, datetime(m.date/1000000000 + 978307200, 'unixepoch', 'localtime') as timestamp, m.is_from_me, h.id as contact, m.text FROM message m LEFT JOIN handle h ON m.handle_id = h.rowid WHERE m.text IS NOT NULL ORDER BY m.date DESC LIMIT ${limit}`;
      }

      try {
        const result = execSync(`sqlite3 -header -separator '|' "${dbPath}" "${query}"`, {
          encoding: 'utf-8',
          timeout: 10000,
        });
        if (!result.trim()) return 'No messages found.';
        // Format as readable output
        const lines = result.trim().split('\n');
        const header = lines[0];
        const rows = lines.slice(1).map(line => {
          const [, timestamp, isFromMe, contact, ...textParts] = line.split('|');
          const direction = isFromMe === '1' ? 'You' : (contact || 'Unknown');
          return `[${timestamp}] ${direction}: ${textParts.join('|')}`;
        });
        return rows.reverse().join('\n');
      } catch (err) {
        if (err.message.includes('authorization denied') || err.message.includes('not authorized')) {
          return 'Error: Full Disk Access required. The user needs to grant Full Disk Access to Terminal/IDE in System Settings > Privacy & Security > Full Disk Access.';
        }
...

  {
    ...
    description: 'Search iMessages by keyword across all conversations. Requires Full Disk Access permission.',
    parameters: {
      type: 'object',
      properties: {
        query: { type: 'string', description: 'Search term to look for in message text' },
    ...
    async execute(args) {
      ...
      const searchTerm = args.query.replace(/'/g, "''");

      const query = `SELECT datetime(m.date/1000000000 + 978307200, 'unixepoch', 'localtime') as timestamp, m.is_from_me, h.id as contact, m.text FROM message m LEFT JOIN handle h ON m.handle_id = h.rowid WHERE m.text LIKE '%${searchTerm}%' ORDER BY m.date DESC LIMIT ${limit}`;
      ...
      try {
        ...
        if (!result.trim()) return `No messages found matching "${args.query}".`;
        const lines = result.trim().split('\n');
        const rows = lines.slice(1).map(line => {
          const [timestamp, isFromMe, contact, ...textParts] = line.split('|');
          const direction = isFromMe === '1' ? 'You' : (contact || 'Unknown');
  ...
  {
    name: 'calendar_today',
    description: 'Get today\'s events across all Google Calendar calendars. Shows times, titles, locations, and attendees.',
    parameters: {
      type: 'object',
      properties: {},
      required: [],
    ...
    async execute() {
      try {
        ...
        const events = await getUpcoming(hoursLeft);
...

  {
    ...
    description: 'Get upcoming Google Calendar events in the next N hours (default 24). Shows times, titles, locations, and attendees across all calendars.',
    parameters: {
      type: 'object',
      properties: {
        hours: { type: 'number', description: 'Hours ahead to look (default 24)' },
      },
      required: [],
    },
    async execute(args) {
      try {
        const events = await getUpcoming(args.hours || 24);
        return formatEvents(events);
      } catch (err) {
        return `Calendar error: ${err.message}`;
      }
...

  {
    ...
    description: 'Create a new Google Calendar event. Supports timed events and all-day events.',
    parameters: {
      type: 'object',
      properties: {
        title: { type: 'string', description: 'Event title/summary' },
        start: { type: 'string', description: 'Start time (ISO 8601, e.g. "2025-03-15T14:00:00") or date for all-day ("2025-03-15")' },
        end: { type: 'string', description: 'End time (ISO 8601). Defaults to 1 hour after start if omitted.' },
    ...
    async execute(args) {
      try {
        const event = await createEvent(args.calendar, {
          summary: args.title,
          start: args.start,
          end: args.end,
          description: args.description,
        ...
        const time = event.start?.dateTime
          ? new Date(event.start.dateTime).toLocaleString('en-US', { dateStyle: 'medium', timeStyle: 'short' })
          : event.start?.date || '';
        return `Event created: "${event.summary}" at ${time} [id: ${event.id}]`;
      } catch (err) {
        return `Calendar error: ${err.message}`;
      }
    },
...

  {
    ...
    description: 'Create a Google Calendar event using natural language. Google parses the text to extract date, time, and title. Examples: "Lunch with Bob tomorrow at noon", "Team standup every Monday 9am".',
    parameters: {
      type: 'object',
      properties: {
    ...
    async execute(args) {
      try {
        const event = await quickAdd(args.calendar, args.text);
        const time = event.start?.dateTime
          ? new Date(event.start.dateTime).toLocaleString('en-US', { dateStyle: 'medium', timeStyle: 'short' })
          : event.start?.date || '';
        return `Event created: "${event.summary}" at ${time} [id: ${event.id}]`;
...

  {
    ...
    description: 'Update an existing Google Calendar event. Pass only the fields you want to change.',
    parameters: {
      type: 'object',
      properties: {
        event_id: { type: 'string', description: 'Event ID (from calendar_today or calendar_upcoming output)' },
        calendar: { type: 'string', description: 'Calendar ID (defaults to primary)' },
    ...
    async execute(args) {
      try {
        const changes = {};
        if (args.title !== undefined) changes.summary = args.title;
        if (args.start !== undefined) changes.start = args.start;
        if (args.end !== undefined) changes.end = args.end;
        if (args.description !== undefined) changes.description = args.description;
        if (args.location !== undefined) changes.location = args.location;
        const event = await updateEvent(args.calendar, args.event_id, changes);
        return `Event updated: "${event.summary}" [id: ${event.id}]`;
      } catch (err) {
        return `Calendar error: ${err.message}`;
      }
    },
...

  {
    ...
    async execute(args) {
      try {
        await deleteEvent(args.calendar, args.event_id);
  ...
  {
    name: 'notify_owner',
    description: 'Send a tagged notification to your owner about this BetterFriends conversation. Only works in BetterFriends sessions. Use when something important needs human attention — e.g. a friend\'s bot is asking for a decision, coordination is needed, or urgent info came in. Does not interrupt the owner\'s main session.',
    parameters: {
      type: 'object',
      properties: {
        message: { type: 'string', description: 'The notification message to send to your owner' },
      },
      required: ['message'],
    },
    async execute(args, session) {
      if (!session?._bfHandle) return 'Error: notify_owner only works in BetterFriends sessions.';
      try {
        return await notifyOwnerFromBf(session._bfHandle, args.message);
      } catch (err) {
        return `Notification error: ${err.message}`;
      }
    },
  ...
  {
    name: 'update_config',
    description: 'Update a key in ~/.betterbot/config.json. Use dot notation for nested keys (e.g. "telegram.allowedChatIds"). The value is merged into the existing config — it does not replace the whole file.',
    parameters: {
      type: 'object',
      properties: {
...
The "prompt" is what the disposable agent receives as its task. Write it like instructions: "Check my email inbox and summarize unread messages in the journal."`,
  ...
  // --- User Rules ---
  // Persistent rules that override all other instructions. These appear at the top of the system prompt
  // in every session (chat, heartbeat, ACT, ESCALATE). Use for hard constraints the user tells you.

  {
    name: 'add_rule',
    description: 'Add a persistent rule that applies across ALL sessions (chat, heartbeat, background agents). Rules override all other instructions. Use this when the user says "never do X", "always do Y", or gives you a hard constraint. Rules are different from personality — they are mandatory instructions, not style preferences.',
    parameters: {
      type: 'object',
      properties: {
        rule: { type: 'string', description: 'The rule to add (e.g. "Never touch or modify the gravity repository", "Always ask before sending emails", "Do not act on GitHub notifications from work repos")' },
      },
      required: ['rule'],
    },
    async execute(args, session) {
      try {
        const rules = await addRule(args.rule);
        if (session?._rebuildSystemPrompt) await session._rebuildSystemPrompt();
        return `Rule added (${rules.length} total):\n${rules.map((r, i) => `  ${i}. ${r}`).join('\n')}`;
      } catch (err) {
        return `Error: ${err.message}`;
      }
    },
  ...
  {
    name: 'list_rules',
    description: 'List all persistent user rules. These are mandatory instructions that apply across every session.',
    parameters: { type: 'object', properties: {}, required: [] },
    async execute() {
      try {
        const rules = await listRules();
        if (rules.length === 0) return 'No rules set. Use add_rule to create persistent constraints.';
        return rules.map((r, i) => `${i}. ${r}`).join('\n');
      } catch (err) {
        return `Error: ${err.message}`;
      }
    },
  ...
  {
    name: 'remove_rule',
    description: 'Remove a persistent user rule by its index number. Use list_rules first to see indices.',
    parameters: {
      type: 'object',
      properties: {
        index: { type: 'number', description: 'The index of the rule to remove (from list_rules)' },
      },
      required: ['index'],
    },
    async execute(args, session) {
      try {
        const removed = await removeRule(args.index);
        if (session?._rebuildSystemPrompt) await session._rebuildSystemPrompt();
...
- run_background({ command: "npx tailwindcss --watch", cwd: "/Users/me/myproject", label: "tailwind" })`,
  ...
  {
    name: 'send_to_friend',
    description: 'Send a message to a friend\'s bot via BetterFriends relay. Both sides must be accepted friends. Respects trust tiers.',
    parameters: {
      type: 'object',
      properties: {
        handle: { type: 'string', description: 'Friend handle (e.g. @manuel)' },
    ...
    async execute(args) {
      try {
        const result = await sendToFriend(args.handle, args.message);
        return `Message sent to ${args.handle} (id: ${result.message_id})`;
      } catch (err) {
        return `Failed to send to ${args.handle}: ${err.message}`;
      }
    },
  },
  ...
  {
    name: 'ask_friend',
    description: 'Ask a question to a friend\'s bot. Sends as type "query" so the receiving bot knows to answer directly.',
    parameters: {
      type: 'object',
      properties: {
        handle: { type: 'string', description: 'Friend handle (e.g. @manuel)' },
    ...
    async execute(args) {
      try {
        const result = await sendToFriend(args.handle, args.question, 'query');
        return `Query sent to ${args.handle} (id: ${result.message_id}). They'll respond when their bot processes it.`;
      } catch (err) {
        return `Failed to ask ${args.handle}: ${err.message}`;
      }
    },
  },
  ...
  {
    name: 'list_friends',
    description: 'List connected BetterFriends — shows handles, display names, status, and trust tiers.',
    parameters: { type: 'object', properties: {} },
    async execute() {
      try {
        const data = await listBetterFriends();
        if (!data.friends?.length && !data.pending_requests?.length) {
          return 'No friends yet. Use the relay to send friend requests.';
        }
        const lines = [];
        if (data.friends?.length) {
          lines.push('Friends:');
          for (const f of data.friends) {
            lines.push(`  ${f.handle} (${f.display_name || 'unnamed'}) — ${f.status}, tier: ${f.trust_tier}`);
          }
        }
        if (data.pending_requests?.length) {
          lines.push('Pending requests (from others):');
          for (const r of data.pending_requests) {
            lines.push(`  ${r.handle} (${r.display_name || 'unnamed'}) — sent ${r.created_at}`);
          }
        }
        return lines.join('\n');
      } catch (err) {
        return `Failed to list friends: ${err.message}`;
      }
    },
  },
  ...
  {
    name: 'check_version',
    description: 'Check what version/commit of BetterBot is running and whether it\'s up to date with the remote.',
    parameters: { type: 'object', properties: {}, required: [] },
    async execute() {
      const lines = [];
      // Always try package.json version
      try {
        const pkg = JSON.parse(await readFile(join(TOOLS_APP_ROOT, 'package.json'), 'utf-8'));
        lines.push(`Version: ${pkg.version}`);
      } catch {}

      const repoDir = findRepoDir();
      if (!repoDir) {
        lines.push('Git repo not found — cannot check commit or update status.');
        return lines.join('\n');
      }

      try {
        const hash = execSync('git rev-parse --short HEAD', { cwd: repoDir, encoding: 'utf8', stdio: 'pipe' }).trim();
        const date = execSync('git log -1 --format=%ci', { cwd: repoDir, encoding: 'utf8', stdio: 'pipe' }).trim();
        ...
        try {
          ...
          const behind = execSync('git rev-list --count HEAD..@{u}', { cwd: repoDir, encoding: 'utf8', stdio: 'pipe' }).trim();
          const behindCount = parseInt(behind, 10) || 0;
          lines.push(`Behind remote: ${behindCount}`);
          lines.push(`Up to date: ${behindCount === 0 ? 'yes' : 'no'}`);
        } catch {
          lines.push('Could not check remote status (fetch failed).');
        }
      } catch (err) {
  ...
  {
    name: 'view_changelog',
    description: 'View recent BetterBot commits (changelog). Shows commit hashes, dates, and messages.',
    parameters: {
      type: 'object',
      properties: {
    ...
    async execute(args) {
      const repoDir = findRepoDir();
      if (!repoDir) return 'Git repo not found — cannot view changelog.';

      const count = Math.min(Math.max(parseInt(args.count, 10) || 10, 1), 50);
      try {
        const log = execSync(`git log --format="%h %ad %s" --date=short -${count}`, {
...
// Truncate tool output to prevent context poisoning.
// Strategy: keep first few lines (context) + last chunk (results/summary).
// Most commands print the important stuff at the end.
// Full output saved to a temp file so the agent can read_file if it needs more.
const MAX_TOOL_OUTPUT = 4000; // ~4k chars ≈ ~1k tokens
const HEAD_LINES = 5;         // keep first 5 lines for context
const TAIL_CHARS = 3000;      // keep last ~3k chars (where results usually are)

async function truncateOutput(toolName, result) {
  if (typeof result !== 'string' || result.length <= MAX_TOOL_OUTPUT) return result;

  // Save full output to temp file so agent can access it if needed
  const { mkdir: mkdirAsync, writeFile: writeFileAsync } = await import('node:fs/promises');
  const tmpDir = join(config.dataDir, 'tool-output');
  await mkdirAsync(tmpDir, { recursive: true });
  const stamp = `${toolName}-${Date.now()}`;
  const tmpPath = join(tmpDir, `${stamp}.txt`);
  await writeFileAsync(tmpPath, result, 'utf-8');

  const lines = result.split('\n');
  const head = lines.slice(0, HEAD_LINES).join('\n');
  const tail = result.slice(-TAIL_CHARS);
  const totalLines = lines.length;
  const droppedChars = result.length - head.length - tail.length;
  return `${head}\n\n[... ${droppedChars} chars / ~${totalLines} lines truncated — full output saved to ${tmpPath} ...]\n\n${tail}`;
}
...
export async function executeTool(name, args, session) {
  ...
  try {
    ...
    return await truncateOutput(name, result);
  } catch (err) {
