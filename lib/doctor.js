...
async function checkConfig() {
  ...
  if (!defaultModel) {
    fail('No default model configured. Run: claw init');
    return false;
  }
  ...
  if (credKey) {
    ...
    if (key) {
      ...
      fail(`Missing API key: ${credKey}. Run: claw creds set ${credKey} <your-key>`);
      return false;
    }
  ...
  if (fix && (broken.length > 0 || duplicates.length > 0)) {
    ...
    for (const item of toQuarantine) {
      try {
        ...
        await writeFile(dst + '.reason', `Quarantined by claw doctor --fix: ${new Date().toISOString()}\n${item.reason}\n`, 'utf-8');
...
export async function runDoctor(args = []) {
  ...
  if (reset_flag) {
    ...
    console.log(`\n${dim}Run 'claw init' to set up again.${reset}`);
    return;
  }
  ...
  if (issues === 0) {
    ...
    console.log(`${yellow}Fixed ${issues} issue(s). Run 'claw doctor' again to verify.${reset}`);
  } else {
    console.log(`${yellow}Found ${issues} issue(s). Run 'claw doctor --fix' to auto-repair.${reset}`);
  }
}
