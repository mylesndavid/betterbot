...
const senders = {
  ...
  async betterfriends(message) {
    // Send to all inner_circle friends
    const { sendToFriend, listFriends } = await import('./channels/betterfriends.js');
    const data = await listFriends();
    const innerCircle = (data.friends || []).filter(f => f.trust_tier === 'inner_circle' && f.status === 'accepted');
    if (innerCircle.length === 0) throw new Error('No inner_circle friends to notify');
    for (const friend of innerCircle) {
      await sendToFriend(friend.handle, message);
    }
    return innerCircle.map(f => f.handle).join(', ');
  },
};
...
/**
 * Send a tagged BetterFriends notification to the owner via Telegram.
 * Does NOT inject context into the main Telegram session.
 */
export async function notifyOwnerFromBf(handle, message) {
  const tagged = `[BF ${handle}] ${message}`;
  await senders.telegram(tagged);
  return `Notification sent to owner via Telegram.`;
}
