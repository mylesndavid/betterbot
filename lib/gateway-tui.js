...
function render() {
  if (!state) return;
  // Full clear on first render or view switch; then just overwrite in-place
  if (firstRender) {
    write(CLEAR);
    firstRender = false;
  }
...
function renderDashboard() {
  ...
  write(HIDE_CURSOR + moveTo(1, 1));

  // Helper: move to row, erase it, then write content
  const writeLine = (row, col, text) => write(moveTo(row, 1) + ERASE_LINE + moveTo(row, col) + text);

  // Header
  const uptime = state.startedAt
    ? formatUptime(Date.now() - new Date(state.startedAt).getTime())
    : '...';
  const title = `${c.bold} BetterBot Gateway${c.reset}`;
  const uptimeStr = `${c.dim}\u25b2 ${uptime}${c.reset}`;
  writeLine(1, pad, title);
  write(moveTo(1, cols - uptime.length - 3) + uptimeStr);

  // Separator
  writeLine(2, pad, c.dim + '\u2500'.repeat(w - 2) + c.reset);
  ...
  const panelStatus = state.server
    ...
    : `${c.dim}\u2013 off${c.reset}`;
  writeLine(row, pad + 1, padRight(`Panel`, 17) + panelStatus);
  row++;
  ...
  const tgStatus = state.telegramStop
    ...
    : `${c.dim}\u2013 off${c.reset}`;
  writeLine(row, pad + 1, padRight(`Telegram`, 17) + tgStatus);
  row++;
  ...
  const hbStatus = state.heartbeatTimer
    ...
    : `${c.dim}\u2013 off${c.reset}`;
  writeLine(row, pad + 1, padRight(`Heartbeat`, 17) + hbStatus);
  row++;
  ...
  const cronStatus = state.cronTimer
    ...
    : `${c.dim}\u2013 off${c.reset}`;
  writeLine(row, pad + 1, padRight(`Crons`, 17) + cronStatus);
  row++;

  // Separator + Activity header
  row++;
  writeLine(row, pad, c.dim + '\u2500'.repeat(w - 2) + c.reset);
  row++;
  writeLine(row, pad + 1, `${c.bold}Activity${c.reset}`);
  row++;
  writeLine(row, pad, c.dim + '\u2500'.repeat(w - 2) + c.reset);
  row++;
  ...
  for (let i = 0; i < logAreaHeight; i++) {
    ...
    if (entry) {
      ...
      const text = entry.text.startsWith('  ')
        ...
        : entry.text;
      writeLine(row + i, pad + 1, `${time}  ${text}`);
    } else {
      writeLine(row + i, 1, '');
    }
  }
  ...
  // Footer
  writeLine(rows, pad, c.dim + '  q quit \u00b7 r heartbeat \u00b7 c crons \u00b7 l logs \u00b7 p panel' + c.reset);
}
...
function renderLogsView() {
  ...
  writeLine(1, pad, `${c.bold} Gateway Logs${c.reset}`);
  ...
  for (let i = 0; i < logAreaHeight; i++) {
    ...
    if (entry) {
      ...
      const text = entry.text.startsWith('  ')
        ? `${c.dim}${entry.text}${c.reset}`
        : entry.text;
      writeLine(lineRow, pad + 1, `${time}  ${text}`);
    } else {
      writeLine(lineRow, 1, '');
    }
  }
  ...
  // Footer
  writeLine(rows, pad, c.dim + `  esc/l back \u00b7 \u2191\u2193 scroll \u00b7 home/end jump${scrollInfo}` + c.reset);
}
