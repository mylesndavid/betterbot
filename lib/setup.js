...
export async function runSetup(target) {
  ...
  try {
    if (target) {
      ...
      // Interactive: `claw setup` — show capability status and pick one
      await runSetupMenu();
    }
  } finally {
    write(SHOW_CURSOR);
...
async function setupTelegram() {
  ...
  if (!chatId) {
    ...
    console.log(`  ${c.dim}claw creds set telegram_chat_id YOUR_CHAT_ID${c.reset}\n`);
  ...
  console.log(`  ${c.dim}Start the gateway with: claw gateway${c.reset}\n`);
}
...
async function setupGitHub() {
  ...
  if (ok) {
    ...
    console.log(`  ${c.dim}Then re-run: claw setup github${c.reset}\n`);
  }
}
