...
async function tuiInput(title, prompt, defaultVal, opts = {}) {
  ...
  return new Promise(resolve => {
    ...
    const onData = (buf) => {
      ...
      raw = raw.replace(/\x1b\[200~/g, '').replace(/\x1b\[201~/g, '');
...
async function runWizard() {
  ...
  if (providerMeta?.credKey) {
    ...
    if (existing) {
      ...
      if (replace) {
        ...
        if (value) {
      // For security and flow, we should validate the key with the provider first.
      // Only save if fetchModels or a ping succeeds.
  ...
  if (!quick) {
    ...
    for (const cred of extraKeys) {
      ...
      if (value) {
        // Validation placeholder - in a real scenario we would check validity here before saving
