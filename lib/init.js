...
async function tuiInput(title, prompt, defaultVal, opts = {}) {
  ...
  return new Promise(resolve => {
    ...
    const onData = (buf) => {
      ...
      // Skip arrow keys and other escape sequences
...
async function runIdentityChat(identityPath, overrides) {
  ...
  try {
    ...
    write(moveTo(chatRow++, pad) + c.dim + 'Skipping identity chat. You can run claw init again later.' + c.reset);
...
`;
    ...
    console.log(`  ${c.dim}Run claw init again to set it up later.${c.reset}`);
    console.log();
  }
...
async function runWizard() {
  ...
  // npm link
  const doLink = await tuiConfirm('Install CLI', 'Install `claw` command globally? (may need sudo)', true);
  if (doLink) {
    write(CLEAR + SHOW_CURSOR);
    console.log('\n  Linking claw command...\n');
    try {
      execSync('npm link', { cwd: resolve(__dirname, '..'), stdio: 'inherit' });
    } catch {
      console.log(`\n  ${c.yellow}Link failed — try: sudo node bin/claw init${c.reset}`);
    }
    console.log(`\n  ${c.dim}Press any key${c.reset}`);
    if (process.stdin.setRawMode) process.stdin.setRawMode(true);
    await readKey();
  ...
  drawText(centerR + 5, centerC + 4, `${c.cyan}claw chat${c.reset}      — talk to ${config.agentName}`);
  drawText(centerR + 6, centerC + 4, `${c.cyan}claw panel${c.reset}     — control panel`);
  drawText(centerR + 7, centerC + 4, `${c.cyan}claw heartbeat${c.reset} — background check-ins`);
