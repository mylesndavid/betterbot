...
function listEntries(dir, { showHidden = false, extensions = ['.zip'] } = {}) {
  try {
    return readdirSync(dir)
      .filter(name => {
        if (!showHidden && name.startsWith('.')) return false;
        try {
          ...
          if (s.isFile() && extensions.some(ext => name.endsWith(ext))) return true;
        } catch {}
        return false;
      })
      .map(name => {
        try {
          return { name, isDir: statSync(join(dir, name)).isDirectory() };
        } catch {
...
async function tuiBrowseFile(startPath, opts = {}) {
  const extensions = opts.extensions || ['.zip'];
  let current = startPath || home;
  let selected = 0;
  let scroll = 0;
  let showHidden = false;
  ...
  function draw() {
    ...
    // Title bar
    write(moveTo(1, 1) + c.bgMagenta + c.white + c.bold + padRight('  Browse for File', cols) + c.reset);

    // Sidebar
    write(moveTo(2, 1) + c.yellow + c.bold + ' Shortcuts' + c.reset);
    const validBookmarks = FILE_BOOKMARKS.filter(b => existsSync(b.path));
    for (let i = 0; i < validBookmarks.length; i++) {
      const bm = validBookmarks[i];
      let style;
      if (sidebarFocused && i === sidebarSelected) {
    ...
    // Entry list
    const entries = listEntries(current, { showHidden, extensions });
    const allItems = [{ name: '..', isDir: true, isParent: true }, ...entries.map(e => ({ ...e, isParent: false }))];
    ...
    for (let i = 0; i < listH; i++) {
      ...
      if (i < visible.length) {
        const item = visible[i];
        const idx = scroll + i;
        const icon = item.isParent ? '↩' : item.isDir ? '▸' : '◆';
        const name = item.isParent ? '..' : item.isDir ? item.name + '/' : item.name;
        if (!sidebarFocused && idx === selected) {
          write(moveTo(row, col) + c.inverse + ` ${icon} ${padRight(name, mainW - 5)}` + c.reset);
        } else {
          const color = item.isParent ? c.dim : item.isDir ? c.white : c.green;
          write(moveTo(row, col) + `${color} ${icon} ${name}${c.reset}`);
        }
      } else {
        write(moveTo(row, col) + clearLine);
    ...
    // Status bar
    write(moveTo(rows - 1, 1) + c.bgGray + c.white + padRight(
      `  ↑↓ navigate  → enter folder  ← go up  Tab sidebar  . toggle hidden  Enter select file  Esc cancel`,
      cols
    ) + c.reset);

    // Selected item hint
    const sel = allItems[selected];
    const hint = sel && !sel.isParent && !sel.isDir ? join(current, sel.name) : current;
    write(moveTo(rows - 2, 1) + c.green + `  ${sel && !sel.isDir && !sel.isParent ? 'File' : 'Dir'}: ${hint}` + c.reset);
  }
...
async function runWizard() {
  ...
  const action = await tuiSelect('Welcome to BetterBot', [
    ...
    { label: 'Restore from Backup', value: 'restore', hint: 'import a betterbot-export .zip' },
    { label: 'Quit', value: 'quit' },
  ]);
  if (!action || action === 'quit') return;

  if (action === 'restore') {
    const method = await tuiSelect('Restore from Backup', [
      { label: 'Browse for file', value: 'browse', hint: 'arrow keys to navigate' },
      { label: 'Type path manually', value: 'type' },
    ]);
    if (!method) return;
    let zipPath;
    if (method === 'browse') {
      zipPath = await tuiBrowseFile(join(home, 'Downloads'));
    } else {
      zipPath = await tuiInput('Restore from Backup', 'Path to .zip file:');
    }
    if (zipPath) {
      const resolved = resolve(zipPath.replace(/^~/, home));
      const { importState } = await import('./export-import.js');
      if (process.stdin.setRawMode) process.stdin.setRawMode(false);
      write(SHOW_CURSOR + '\n');
      await importState(resolved);
      write('\n');
    }
    return;
  }
