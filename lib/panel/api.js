...
import { listCredentials, setCredential, removeCredential, getCredential } from '../credentials.js';
...
import { loadGraph, reloadGraph } from '../graph-memory.js';
...
async function fetchProviderModels(provider) {
  const credKey = PROVIDER_CRED_MAP[provider];
  const apiKey = credKey ? await getCredential(credKey).catch(() => null) : null;

  try {
    switch (provider) {
      case 'anthropic': {
        if (!apiKey) return MODEL_FALLBACKS.anthropic;
        const res = await fetch('https://api.anthropic.com/v1/models', {
          headers: { 'x-api-key': apiKey, 'anthropic-version': '2023-06-01' },
        });
        if (!res.ok) return MODEL_FALLBACKS.anthropic;
      ...
      case 'openai': {
        if (!apiKey) return MODEL_FALLBACKS.openai;
        const res = await fetch('https://api.openai.com/v1/models', {
          headers: { 'Authorization': `Bearer ${apiKey}` },
        });
        if (!res.ok) return MODEL_FALLBACKS.openai;
        const data = await res.json();
        return (data.data || [])
      ...
      case 'openrouter': {
        ...
        if (!res.ok) return MODEL_FALLBACKS.openrouter;
        const data = await res.json();
        return (data.data || [])
      ...
      case 'ollama': {
        ...
        if (!res.ok) return MODEL_FALLBACKS.ollama;
        const data = await res.json();
        return (data.models || [])
      ...
      case 'together': {
        if (!apiKey) return MODEL_FALLBACKS.together;
        const res = await fetch('https://api.together.xyz/v1/models', {
          headers: { 'Authorization': `Bearer ${apiKey}` },
        });
        if (!res.ok) return MODEL_FALLBACKS.together;
        const data = await res.json();
        return (data || [])
      ...
      case 'groq': {
        if (!apiKey) return MODEL_FALLBACKS.groq;
        const res = await fetch('https://api.groq.com/openai/v1/models', {
          headers: { 'Authorization': `Bearer ${apiKey}` },
        });
        if (!res.ok) return MODEL_FALLBACKS.groq;
        const data = await res.json();
        return (data.data || [])
          .map(m => ({ id: m.id, name: m.id }))
          .sort((a, b) => a.id.localeCompare(b.id));
  ...
  try {
    ...
    if (pathname === '/api/gateway' && req.method === 'GET') {
      ...
      return json(res, {
        ...
        telegram: {
          connected: !!gatewayState.telegramStop,
          botName: gatewayState.telegramBotName || null,
        },
      });
    }

    // POST /api/doctor — run headless doctor
    ...
    // POST /api/channels/test — send a test notification
    if (pathname === '/api/channels/test' && req.method === 'POST') {
      const { channel } = await parseJSON(req);
      const { notifyUser } = await import('../notify.js');
      await notifyUser('Test notification from BetterClaw panel', { channel });
    ...
    // GET /api/models/:provider — fetch available models from provider API
    if (pathname.startsWith('/api/models/') && req.method === 'GET') {
      const provider = pathname.slice('/api/models/'.length);
    ...
    // POST /api/graph/reload — force re-read from disk
    if (pathname === '/api/graph/reload' && req.method === 'POST') {
      try {
        const graph = await reloadGraph();
        const data = graph.export();
        return json(res, { ok: true, nodes: data.nodes.length, edges: data.edges.length });
      } catch (err) {
    ...
    if (pathname === '/api/graph/search' && req.method === 'GET') {
      try {
        const url = new URL(req.url, 'http://localhost');
    ...
    // GET /api/graph/node/:id — get node details and neighbors
    if (pathname.startsWith('/api/graph/node/') && req.method === 'GET') {
      try {
        const nodeId = decodeURIComponent(pathname.slice('/api/graph/node/'.length));
        const graph = await loadGraph();
        ...
        const neighborhood = graph.traverse(nodeId, 1);

        return json(res, {
          id: nodeId,
          ...node,
          neighbors: neighbors.map(id => ({ id, ...graph.getNode(id) })),
          edges: edges,
          neighborhood: neighborhood.map(n => ({ id: n.id, ...n.attrs, depth: n.depth })),
        });
      } catch (err) {
        return error(res, err.message, 500);
      }
    ...
    // GET /api/graph/traverse/:id?hops= — traverse from a node
    if (pathname.startsWith('/api/graph/traverse/') && req.method === 'GET') {
      try {
        const url = new URL(req.url, 'http://localhost');
        const nodeId = decodeURIComponent(pathname.slice('/api/graph/traverse/'.length));
        const hops = parseInt(url.searchParams.get('hops') || '2', 10);

        const graph = await loadGraph();
        const traversal = graph.traverse(nodeId, hops);
        const result = traversal.map(n => ({
          id: n.id,
          ...n.attrs,
          depth: n.depth,
        }));
        return json(res, { nodes: result });
      } catch (err) {
        return error(res, err.message, 500);
      }
    }
