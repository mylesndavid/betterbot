...
async function relay(method, path, body, token) {
  const url = `${config.betterfriends?.relayUrl}${path}`;
  const opts = {
    method,
    headers: {
...
async function getSession(handle, sessionMap) {
...
async function handleIncomingMessage(msg, token, sessionMap) {
  ...
  const session = await getSession(handle, sessionMap);

  // Format incoming message with sender identity
  const prefix = `[BetterFriend ${handle} (${displayName})]`;
  let text;
  if (msg.type === 'query') {
    text = `${prefix} asks: ${msg.content}`;
  } else if (msg.type === 'status_request') {
    text = `${prefix} wants to know your status: ${msg.content}`;
  } else {
      ...
      try {
        ...
        if (data.messages?.length > 0) {
          ...
          try {
            ...
            console.error(`BetterFriends: ack failed: ${err.message}`);
          }
        }
      } catch (err) {
        if (!running) break;
        console.error(`BetterFriends poll error: ${err.message}`);
...
export async function setupBetterFriends() {
  ...
  relayUrl = relayUrl.trim() || 'http://localhost:4444';
...
export async function sendToFriend(handle, content, type = 'message', metadata = null) {
  const token = await getCredential('betterfriends_token');
  if (!token) throw new Error('BetterFriends not configured');
  return relay('POST', '/api/send', { to: handle, content, type, metadata }, token);
}
