...
// Module-level session map so exports can access it
...
Your reply is sent back to ${handle} automatically — do NOT use send_to_friend or ask_friend tools.
If something important needs your owner's attention, use the notify_owner tool to send them a tagged notification.
Your owner can join this conversation via the /bf command — their messages will be prefixed with [Owner].
...
async function getSession(handle, displayName, sessionMap) {
  ...
  // Inject channel context, tool restrictions, and handle marker
...
export async function startBetterFriends() {
  ...
  _sessionMap = await loadSessionMap();
...
/**
 * Send a message from the owner into a BetterFriends session.
 * Routes through the agent (session.sendStream), then relays the response to the friend.
 * Returns the agent's response text.
 */
export async function sendOwnerMessage(handle, message) {
  const token = await getCredential('betterfriends_token');
  if (!token) throw new Error('BetterFriends not configured. Run: betterbot setup betterfriends');

  // Load session map if not already loaded (gateway might not be running)
  if (!_sessionMap) _sessionMap = await loadSessionMap();

  // Resolve display name from friends list
  let displayName = handle;
  try {
    const data = await relay('GET', '/api/friends', null, token);
    const friend = data.friends?.find(f => f.handle === handle);
    if (friend) displayName = friend.display_name || handle;
  } catch {}

  const session = await getSession(handle, displayName, _sessionMap);

  // Send as owner — prefix so the agent knows the human is talking
  let fullResponse = '';
  try {
    for await (const event of session.sendStream(`[Owner] ${message}`)) {
      if (event.type === 'text') {
        fullResponse += event.text;
      }
    }
  ...
  // Relay the response to the friend
  if (fullResponse.trim()) {
    try {
      await relay('POST', '/api/send', {
        to: handle,
      ...
      fullResponse += `\n(Failed to relay to ${handle}: ${err.message})`;
    }
  }
