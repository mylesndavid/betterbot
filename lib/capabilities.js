...
const capabilities = [
...

  {
    ...
    async check() {
      ...
      return {
        ...
        setup: 'User needs to: 1) Create a bot via @BotFather on Telegram, 2) Store the token with store_credential("telegram_bot_token", "..."), 3) Send a message to the bot, then set their chat ID in config. Run "betterbot telegram setup" for guided setup.',
      };
    },
  },
  ...
  {
    name: 'Browser',
    description: 'Browse websites, interact with pages, extract content, record sessions as MP4 video (ARIA snapshots, no vision needed)',
    async check() {
      ...
      // Soft check for ffmpeg (recording is optional)
      let hasFFmpeg = false;
      try {
      ...
      return {
        ready: true,
        missing: hasFFmpeg ? undefined : 'ffmpeg not installed (optional — needed for browse_web recording)',
      };
    },
  },

  {
    ...
    description: 'Read, create, update, and delete Google Calendar events. Heartbeat checks upcoming events.',
    async check() {
      const refreshToken = await getCredential('google_refresh_token');
      if (refreshToken) return { ready: true };
      ...
      if (!refreshToken) missing.push('google_refresh_token (run: betterbot auth google)');
      return {
        ready: false,
        missing: missing.join(', '),
        setup: 'Run "betterbot auth google" to connect Google Calendar via OAuth2. Requires google_client_id and google_client_secret credentials first — get these from Google Cloud Console (APIs & Services > Credentials > OAuth 2.0 Client ID, type Desktop).',
      };
    },
  },
  ...
  {
    name: 'iMessage',
    description: 'Send and read iMessages via Messages app and chat.db',
    async check() {
      const { execSync } = await import('node:child_process');
      ...
      // Test AppleScript access to Messages app
      try {
        execSync('osascript -e \'tell application "Messages" to name\'', {
          encoding: 'utf-8', timeout: 10000,
        });
        canSend = true;
      } catch {
        missing.push('Messages Automation permission (System Settings > Privacy & Security > Automation)');
      }

      // Test chat.db read access
      try {
        const dbPath = join(homedir(), 'Library/Messages/chat.db');
        execSync(`sqlite3 "${dbPath}" "SELECT COUNT(*) FROM message LIMIT 1"`, {
          encoding: 'utf-8', timeout: 5000,
        });
        canRead = true;
      } catch {
        missing.push('Full Disk Access for reading messages (System Settings > Privacy & Security > Full Disk Access)');
      }

      if (canSend && canRead) return { ready: true };
      if (canSend) return { ready: true, missing: missing.join(', ') }; // send works, read is optional
      return {
        ready: false,
        missing: missing.join(', '),
        setup: 'Run "betterbot setup imessage" to test permissions. The user needs to grant Automation (for sending) and Full Disk Access (for reading) in macOS System Settings.',
      };
    },
  },
  ...
  {
    name: 'BetterFriends',
    description: 'Communicate with other BetterBot agents via the BetterFriends relay',
    async check() {
      const token = await getCredential('betterfriends_token');
      const relayUrl = config.betterfriends?.relayUrl;
      if (token && relayUrl) return { ready: true };
      const missing = [];
      if (!token) missing.push('betterfriends_token credential');
      if (!relayUrl) missing.push('betterfriends.relayUrl in config');
      return {
        ready: false,
        missing: missing.join(', '),
        setup: 'Run "betterbot setup betterfriends" to register on a relay, store the auth token, and configure the relay URL.',
      };
    },
  },

  {
    ...
    async check() {
      ...
      return {
        ready: false,
        missing: 'slack_bot_token credential',
        setup: 'User creates a Slack app at api.slack.com/apps, adds chat:write + channels:read + channels:history + users:read scopes, installs to workspace, stores bot token with store_credential("slack_bot_token"). Tools are built-in — no custom tools needed.',
      };
    },
  },
];
