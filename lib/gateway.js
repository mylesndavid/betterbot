...
// ── Auto-update: check git remote for new commits, pull + restart ──

let _repoDir = null; // cached git repo location

function findRepoDir() {
  if (_repoDir !== null) return _repoDir;
  const candidates = [
    APP_ROOT,
...
async function checkForUpdates() {
  ...
  try {
    execSync('git fetch --quiet', { cwd: repoDir, stdio: 'pipe', timeout: 15000 });

    const local = execSync('git rev-parse HEAD', { cwd: repoDir, encoding: 'utf8', stdio: 'pipe' }).trim();
    const remote = execSync('git rev-parse @{u}', { cwd: repoDir, encoding: 'utf8', stdio: 'pipe' }).trim();

    if (local === remote) return;

    const count = execSync(`git rev-list --count ${local}..${remote}`, { cwd: repoDir, encoding: 'utf8', stdio: 'pipe' }).trim();
    logEvent(`Auto-update: ${count} new commit${count === '1' ? '' : 's'} — pulling...`);

    const status = execSync('git status --porcelain', { cwd: repoDir, encoding: 'utf8', stdio: 'pipe' }).trim();
    if (status) {
      logEvent('Auto-update: skipped — uncommitted changes');
      return;
    }

    execSync('git pull --ff-only', { cwd: repoDir, stdio: 'pipe', timeout: 30000 });

    // Re-link global install
    try {
      execSync('npm install -g .', { cwd: repoDir, stdio: 'pipe', timeout: 60000 });
    } catch {}

    const log = execSync(`git log --oneline ${local}..${remote}`, { cwd: repoDir, encoding: 'utf8', stdio: 'pipe' }).trim();
    logEvent(`Auto-update: updated\n${log}`);

    // Exit cleanly — LaunchAgent restarts automatically (KeepAlive)
    logEvent('Auto-update: restarting...');
    setTimeout(() => process.exit(0), 1000);
  } catch {
    // Silent — don't spam logs if offline
  }
}
...
export async function startGateway(opts = {}) {
  ...
  // 2b. Start BetterFriends channel (if relay configured)
  if (config.betterfriends?.relayUrl) {
    try {
      const { startBetterFriends } = await import('./channels/betterfriends.js');
      gatewayState.betterfriendsStop = await startBetterFriends();
      gatewayState.betterfriendsHandle = gatewayState.betterfriendsStop.handle || null;
      logEvent(`BetterFriends: connected${gatewayState.betterfriendsHandle ? ` (${gatewayState.betterfriendsHandle})` : ''}`);
    } catch (err) {
      logEvent(`BetterFriends: skipped (${err.message})`);
    }
  }
  ...
  // 5. Auto-update — check for new commits every 5 minutes
  gatewayState.updateTimer = setInterval(checkForUpdates, 5 * 60 * 1000);
  ...
  const shutdown = async (signal) => {
    ...
    if (gatewayState.updateTimer) clearInterval(gatewayState.updateTimer);
    if (gatewayState.telegramStop) gatewayState.telegramStop();
    if (gatewayState.betterfriendsStop) gatewayState.betterfriendsStop();
