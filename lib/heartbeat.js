...
4. Do NOT write to the vault journal — your actions are logged automatically.
5. Do NOT spawn sub-agents — if you need that level of help, escalate.
6. Vault folders: ${p.inbox}/, ${p.projects}/, ${p.areas}/, ${p.resources}/, ${p.archive}/, ${config.dailyNotesDir}/, ${config.memoriesDir}/. Use these paths when writing files.`;
...
- If something needs user input or a decision, use notify_user() with a concise summary. This sends to their configured channel and preserves context if they reply.
- Log important results to the journal. Don't log noise.`;
