...
function annotateEvents(events, state) {
  ...
  for (const event of events) {
    if (event.type === 'idle') continue; // idle events re-evaluate every heartbeat
...
// Idle events always count as new — they should re-evaluate each heartbeat
function hasNewContext(events) {
  return events.some(e => !e._prior || e.type === 'idle');
}
...
// Check GitHub activity using the configured gh_token (not the system gh CLI)
// Looks at recent pushes and open PRs across repos the token can access
async function checkGitHub(state) {
  try {
    const token = await getCredential('gh_token');
    if (!token) return [];
    ...
    // Get authenticated user's identity so we can filter out own pushes
    const selfNames = new Set();
    try {
      const userRes = await fetch('https://api.github.com/user', { headers, signal: AbortSignal.timeout(5000) });
      if (userRes.ok) {
        const user = await userRes.json();
        if (user.login) selfNames.add(user.login.toLowerCase());
        if (user.name) selfNames.add(user.name.toLowerCase());
      }
    } catch {}
    ...
    // Check each repo for new commits and open PRs (parallel, capped at 5 repos)
    const checks = repos.slice(0, 5).map(async (repo) => {
      const items = [];
      try {
        ...
        if (commitsRes.ok) {
          ...
          for (const c of commits) {
            ...
            items.push({
              type: 'github',
              summary: `GitHub push: ${msg} (${repo.full_name} by ${author})`,
              _id: id,
            });
          }
        }
        ...
        if (prsRes.ok) {
          ...
          for (const pr of prs) {
            const id = `pr:${repo.full_name}#${pr.number}`;
            allIds.push(id);
            if (seenSet.has(id)) continue;
            items.push({
              type: 'github-pr',
              summary: `PR #${pr.number}: ${pr.title} (${repo.full_name} by ${pr.user?.login})`,
              _id: id,
            });
          }
        }
    ...
    const rawEvents = [];
    for (const items of results) rawEvents.push(...items);

    // Update seen set (keep last 500 to prevent unbounded growth)
    state.seenGitHub = allIds.slice(-500);

    // Filter out own pushes — user already knows what they pushed
    const filteredEvents = selfNames.size > 0
      ? rawEvents.filter(e => {
          if (!e.summary.startsWith('GitHub push:')) return true;
          const authorMatch = e.summary.match(/by ([^)]+)\)$/);
          return !authorMatch || !selfNames.has(authorMatch[1].toLowerCase());
        })
      : rawEvents;

    // Batch commits by repo+author into digest events so triage sees the big picture
    const commitGroups = new Map();
    const nonCommits = [];
    for (const e of filteredEvents) {
      if (e.summary.startsWith('GitHub push:')) {
        const repoMatch = e.summary.match(/\(([^ ]+) by ([^)]+)\)/);
        if (repoMatch) {
          const key = `${repoMatch[1]}|${repoMatch[2]}`;
          if (!commitGroups.has(key)) commitGroups.set(key, { repo: repoMatch[1], author: repoMatch[2], commits: [], ids: [] });
          const msg = e.summary.replace('GitHub push: ', '').replace(/ \([^)]+\)$/, '');
          commitGroups.get(key).commits.push(msg);
          commitGroups.get(key).ids.push(e._id);
        } else {
          nonCommits.push(e);
        }
      } else {
    ...
    const batched = [...nonCommits];
    for (const [, group] of commitGroups) {
      const msgs = group.commits.slice(0, 5).map(m => m.slice(0, 80)).join('; ');
      const count = group.commits.length;
      // Tag with commit count so triage can distinguish busy activity from noise
      const tag = count >= 3 ? 'github-active' : 'github';
      batched.push({
        type: tag,
        summary: `${group.author} pushed ${count} commit${count === 1 ? '' : 's'} to ${group.repo}: ${msgs}`,
        _id: group.ids[0],
      });
    }

    return batched.slice(0, 10);
  } catch {
    return [];
  }
}
...
async function checkCalendar(state) {
  try {
    const { getUpcoming } = await import('./google-calendar.js');
    const events = await getUpcoming(0.5); // next 30 minutes

    if (!state.seenCalendar) state.seenCalendar = [];
    const seenSet = new Set(state.seenCalendar);
    const newEvents = [];
    ...
    for (const event of events) {
      ...
      const minutesUntil = Math.round((start - Date.now()) / 60000);
      const timeLabel = minutesUntil <= 0 ? 'now' : `in ${minutesUntil} min`;

      newEvents.push({
        type: 'calendar',
        summary: `${event.summary || 'Event'} ${timeLabel}${event.location ? ` at ${event.location}` : ''}`,
        _id: event.id,
      });
    }

    // Track seen event IDs (keep last 200)
    const allIds = [...state.seenCalendar, ...newEvents.map(e => e._id)];
    state.seenCalendar = allIds.slice(-200);

    return newEvents;
  } catch {
    return []; // Calendar not configured or API error — non-fatal
  }
}
...
async function triageEvents(events) {
  ...
  // Load user rules so triage respects hard constraints
  const rules = await getUserRules().catch(() => null);
  const rulesBlock = rules ? `\nUSER RULES (mandatory — obey these above all else):\n${rules}\n` : '';
  ...
  const systemPrompt = `You are a triage classifier for an AI assistant's heartbeat. Classify each event. Respond ONLY with a JSON array.

Format: [{"event":"...","action":"IGNORE|LOG|ACT|ESCALATE","reason":"..."}]

- IGNORE: Noise, routine, or already handled.
- LOG: Worth a journal note, no notification needed.
- ACT: Worth telling the user about or taking action on.
- ESCALATE: Complex work needing the full agent with conversation history.
${rulesBlock}
Events marked [PREVIOUSLY ...] were already handled today — IGNORE unless something changed.`;

  // Debug: log triage input and output
  if (process.env.DEBUG_TRIAGE) {
    console.log('=== TRIAGE SYSTEM ===\n' + systemPrompt);
    console.log('=== TRIAGE EVENTS ===\n' + eventList);
  }
...
async function actOnEvents(actEvents) {
  ...
  // Load personality so ACT agent sounds like Raya, not a robot
  let personalityBlock = '';
  try {
    const { getPersonality } = await import('./personality.js');
    const personality = await getPersonality();
    if (personality) personalityBlock = `\n${personality}\n`;
  } catch {}
...
CRITICAL:
- You MUST call notify_user() to send the message — the user can't see your text responses, only tool calls reach them. Include the "context" parameter explaining why you're reaching out (e.g. "Idle check-in: building user profile, asked about current projects") — this gets injected into the conversation session so if the user replies, the main agent knows what's going on.
...
- Only send ONE notify_user() per idle check-in.` : '';
  ...
  const prompt = `You are ${config.agentName}. Write messages in a casual, natural tone — like texting a friend.
${personalityBlock}
HEARTBEAT — ${timeStr}
...
RULES:
1. Use notify_user() to tell the user about interesting events. The user can't see your text — only tool calls reach them. Journal logging alone is NOT enough.
2. Be HONEST. If a tool errors or returns nothing, say so. NEVER claim you did something you didn't.
3. If you can't complete a task — respond with ESCALATE: followed by what you tried and why it didn't work.
4. Do NOT create journal tasks that already exist. Only journal genuinely new findings to the Notes section.
5. Do NOT spawn sub-agents or modify your own personality/settings. If you need more help, escalate.
6. Vault folders: ${p.inbox}/, ${p.projects}/, ${p.resources}/, ${config.dailyNotesDir}/. Use remember() for persistent knowledge.`;
...
export async function runHeartbeat() {
  ...
  if (sources.includes('calendar')) {
    events.push(...await checkCalendar(state));
  }
  if (sources.includes('betterfriends')) {
    try {
      const { checkBetterFriendsMessages } = await import('./channels/betterfriends.js');
      events.push(...await checkBetterFriendsMessages());
    } catch {}
  }
  ...
  if (events.length === 0 && config.heartbeat?.proactive !== false) {
    ...
    if (lastContact) {
      ...
      const isWakingHours = hour >= 7 && hour <= 23;
